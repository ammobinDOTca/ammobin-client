...
require('ammobin-classifier/build/rimfire-calibres').rimfireCalibres
  .map(l => head(l).toUpperCase())
  .forEach(c => ws.write(`${site}rimfire?calibre=${encodeURIComponent(c)}&page=1\n`))

require('ammobin-classifier/build/centerfire-calibres').centerFireCalibres
  .map(l => head(l).toUpperCase())
  .forEach(c => ws.write(`${site}centerfire?calibre=${encodeURIComponent(c)}&page=1\n`))

require('ammobin-classifier/build/shotgun-gauges').shotgunGauges
  .map(l => head(l).toUpperCase())
